...
@Component({
  selector: 'app-workflow-create-alert',
  standalone: true,
  imports: [CommonModule, FormsModule, WorkflowNodeComponent],
  template: `
    <div class="page-header fade-in">
      <div>
        <h1>⚠️ New Alert Workflow</h1>
        <p style="color: var(--color-text-muted); margin: 0;">Get emailed the moment stock at a warehouse drops below a threshold.</p>
      </div>
    ...
    <div class="card wizard-card slide-up">
      <app-workflow-node [stepNumber]="1" title="Select Warehouse" icon="🏭"
        [state]="step() === 1 ? 'active' : (step() > 1 ? 'done' : 'pending')"
        [summary]="step() > 1 ? warehouseName() : ''" [isLast]="false">
        ...
        <button class="btn btn-primary next-btn" [disabled]="!warehouseId" (click)="goTo(2)">Next →</button>
      </app-workflow-node>

      <app-workflow-node [stepNumber]="2" title="Select Product (optional)" icon="🏷️"
        [state]="step() === 2 ? 'active' : (step() > 2 ? 'done' : 'pending')"
        [summary]="step() > 2 ? (productId ? productName() : 'Any product at this warehouse') : ''" [isLast]="false">
        <select class="form-control" [(ngModel)]="productId">
          <option [ngValue]="null">Any product at this warehouse</option>
          <option *ngFor="let p of products()" [ngValue]="p.id">{{ p.name }} ({{ p.sku }})</option>
        </select>
        <div class="btn-row">
          <button class="btn btn-secondary" (click)="goTo(1)">← Back</button>
          <button class="btn btn-primary next-btn" (click)="goTo(3)">Next →</button>
        </div>
      </app-workflow-node>
      ...
      <app-workflow-node [stepNumber]="4" title="Configure Gmail Recipient" icon="📧"
        [state]="step() === 4 ? 'active' : (step() > 4 ? 'done' : 'pending')"
        [summary]="step() > 4 ? recipientEmail : ''" [isLast]="false">
        <label class="field-label">Send the alert email to:</label>
      ...
      <app-workflow-node [stepNumber]="5" title="Review & Activate" icon="🚀"
        ...
        <input class="form-control" [(ngModel)]="name" placeholder="e.g. Main Warehouse Low Stock Alert" />

        <div class="review-box">
          <div class="review-row"><span>Warehouse</span><strong>{{ warehouseName() }}</strong></div>
          <div class="review-row"><span>Product</span><strong>{{ productId ? productName() : 'Any product' }}</strong></div>
...
export class WorkflowCreateAlertComponent implements OnInit {
  ...
  submit(): void {
    if (!this.warehouseId || !this.thresholdQuantity || !this.isValidEmail(this.recipientEmail) || !this.name.trim()) return;
    this.submitting.set(true);
    this.errorMessage.set('');

    this.workflowService.createAlert({
      name: this.name.trim(),
